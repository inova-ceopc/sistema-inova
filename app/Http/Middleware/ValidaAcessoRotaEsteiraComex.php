            ...
            case 'esteiracomex/contratacao/analise/':
                if ($request->session()->get('unidadeEmpregadoEsteiraComex') != '5459') {
                    $request->session()->flash([
                        ...
                        'corpoMensagem' => "Você não pode analisar essa demanda."
                    ]); 
                    ...
                    if ($demanda->responsavelCeopc != $request->session()->get('matricula') || $demanda->responsavelCeopc == null || $demanda->responsavelCeopc == 'NULL') {
                        $request->session()->flash([
                            'corMensagem' => 'warning',
                            'tituloMensagem' => "Protocolo #" . str_pad($id, 4, '0', STR_PAD_LEFT) . " | não pode ser acessado!",
                            'corpoMensagem' => "A demanda não foi distribuida ou está distribuida para outro analista.<br/>Para tratar essa demanda, solicite a distribuição para sua matrícula."
                        ]); 
            ...
            case 'esteiracomex/distribuir':             
                if ($request->session()->get('unidadeEmpregadoEsteiraComex') != '5459') {
                    $request->session()->flash([
                        'corMensagem' => 'warning',
                        'tituloMensagem' => "Acesso negado!",
                        'corpoMensagem' => "Você não tem perfil para distribuir demandas."
                    ]);
                    return redirect('esteiracomex/');
            ...
            case 'esteiracomex/contratacao':
                if ($request->session()->get('unidadeEmpregadoEsteiraComex') == '5459') {
                    $request->session()->flash([
                        'corMensagem' => 'warning',
                        'tituloMensagem' => "Acesso negado!",
                        'corpoMensagem' => "Você não tem perfil para cadastrar novas demandas."
                    ]);
                    return redirect('esteiracomex/');
                }
                break;
            case 'esteiracomex/contratacao/complemento/':
                if ($request->session()->get('unidadeEmpregadoEsteiraComex') == '5459') {
                    $request->session()->flash([
                        'corMensagem' => 'warning',
                        'tituloMensagem' => "Acesso negado!",
                        'corpoMensagem' => "Você não tem perfil para editar essa demanda."
                    ]);
                    return redirect('esteiracomex/');
                } else {
                    $demanda = ContratacaoDemanda::find($request->demanda);
                    if ($demanda->statusAtual != 'INCONFORME') {
                        $request->session()->flash([
                            'corMensagem' => 'warning',
                            'tituloMensagem' => "Protocolo #" . str_pad($id, 4, '0', STR_PAD_LEFT) . " | não pode ser modificado!",
                            'corpoMensagem' => "A demanda ainda está em tratamento. Aguarde a finalização da análise."
                        ]);
                        return redirect('esteiracomex/contratacao/consulta/' . $request->demanda);
                    }
                }
